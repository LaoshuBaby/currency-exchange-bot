...
async def rate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            "❌ 使用方法：/rate [来源货币] [目标货币]\n\n"
            "💡 示例：\n"
            "/rate JPY CNY - 查询日元兑人民币\n"
            "/rate USD CNY - 查询美元兑人民币\n"
            "/rate EUR USD - 查询欧元兑美元\n\n"
            "📋 常用货币代码：\n"
    ...
    if rate:
        ...
        response = f"💱 汇率查询：{from_currency} → {to_currency}\n\n"
        response += f"📊 当前汇率：\n"
        response += f"1 {from_currency} = {rate:.6f} {to_currency}\n\n"
        
        response += f"💰 换算示例：\n"
        for amount in amounts:
            converted = amount * rate
            response += f"{amount:5,d} {from_currency} ≈ {converted:10.2f} {to_currency}\n"
